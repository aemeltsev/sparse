...
static char RCSid[] =
    "@(#)$Header: /local/cvs/freeda/libs/sparse/spTest.c,v 1.1.1.1 2004/02/25 18:15:35 nmkripla Exp $";
#endif
...
static int InterpretCommandLine(int argc, char* argv[]);
static int ReadMatrixFromFile();
static void PrintMatrixErrorMessage(int Error);
static int CheckInAllComplexArrays();
...
int main(int argc, char* argv[])
{
    ...
    double BeginTime, BuildTime, FactorTime, SolveTime, PartitionTime;
    double InitialFactorTime, ConditionTime, DeterminantTime;
    extern char *sbrk();

    /* Begin `main'. */
    time_t begin_main_t, end_main_t;
    time_t start_t, end_t;
    BeginTime = time(&begin_main_t);
    argc = InterpretCommandLine(argc, argv);
...
/* Assure that the Sparse is compatible with this test program.*/
    ...
    do
    {
        ...
        {
            if (StandardInput)
                sprintf(PlotFile, "bef");
            else
                sprintf(PlotFile, "%s.bef", FileName);
            if (NOT spFileMatrix( Matrix, PlotFile, FileName, NO, NO, NO ))
            {   sprintf(ErrMsg,"%s: plotfile `%s'",ProgramName,PlotFile);
                perror( ErrMsg );
            }
        }
#if NO
        ...
        Error = spOrderAndFactor( Matrix, RHS, RelThreshold, AbsThreshold,
                                  DiagPivoting );
        time(&end_t);
        InitialFactorTime = difftime(end_t, start_t);
 ...
 */
        ...
        else
        {
            ...
            time(&end_t);
            ConditionTime = difftime(end_t, start_t);
            PrintMatrixErrorMessage( Error );
#endif
#if PSEUDOCONDITION
            ...
            time(&end_t);
            PartitionTime = difftime(end_t, start_t);
        }
...
/* Solve system of equations Iterations times. */
        for(I = 1; I <= Iterations; I++)
        {
            ...
            spInitialize(Matrix, Init);
            time(&end_t);
            BuildTime += difftime(end_t, start_t);

            time(&start_t);
            Error = spFactor( Matrix );
            time(&end_t);
            FactorTime += difftime(end_t, start_t);
 ...
 */
            ...
            if (Transposed)
                ...
                spSolve( Matrix, RHS, Solution IMAG_VECTORS );
            time(&end_t);
            SolveTime += difftime(end_t, start_t);
        }
...
#endif
            time(&end_t);
            DeterminantTime = difftime(end_t, start_t);
            if (Complex OR RealAsComplex)
            {
...
End:;
    ...
    {
        time(&end_main_t);
        double result = difftime(end_main_t, begin_main_t);
...
static int ReadMatrixFromFile()
{
    ...
    ComplexNumber *pValue, *pInitInfo;
    RealNumber* pElement;
...
/* Read matrix elements. */
    do
    {   if (Count == 0)
            pValue = CheckOutComplexArray( 1000 );
